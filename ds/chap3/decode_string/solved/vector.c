...
enum Status vector_init(struct Vector *vec, int cap) {
  ...
  vec->buffer = (ElementType *)malloc(cap * sizeof(ElementType));
  if (vec->buffer == NULL) {
...
enum Status vector_reserve(struct Vector *vec, int new_cap) {
  ...
  ElementType *p = (ElementType *)realloc(vec->buffer, new_cap * sizeof(ElementType));
  if (p == NULL) {
    return STATUS_ERR_ALLOC;
  }
...
enum Status vector_insert(struct Vector *vec, int pos, ElementType value) {
  int i;
  enum Status err;
...
enum Status vector_push_back(struct Vector *vec, ElementType value) {
  return vector_insert(vec, vec->size, value);
}

enum Status vector_push_front(struct Vector *vec, ElementType value) {
...
void vector_delete_value(struct Vector *vec, ElementType value) {
  int pos = 0;
...
int vector_find_next(struct Vector *vec, ElementType value, int start_pos) {
  int i;
...
int vector_find_first(struct Vector *vec, ElementType value) {
...
void vector_reverse(struct Vector *vec) {
  ...
  for (i = 0; i < vec->size / 2; i++) {
    SWAP(vec->buffer[i], vec->buffer[vec->size - i - 1], ElementType);
  }
}
...
void vector_swap(struct Vector *a, struct Vector *b) {
  ...
  SWAP(a->buffer, b->buffer, ElementType *);
}
...
ElementType vector_at(struct Vector *vec, int index) {
